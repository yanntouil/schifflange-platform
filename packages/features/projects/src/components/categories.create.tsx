 ...
 */
export const CategoriesCreateDialog: React.FC<Ui.QuickDialogProps<void, Api.ProjectCategory>> = ({
  item,
  ...props
}) => {
...
const DialogForm: React.FC<Ui.QuickDialogSafeProps<void, Api.ProjectCategory>> = ({ close, mutate }) => {
  const { _ } = useTranslation(dictionary)
  const { current } = useContextualLanguage()
  const { service } = useProjectsService()
  const initialValues = {
    order: 0,
    translations: useFormTranslatable([] as Api.ProjectCategoryTranslation[], servicePlaceholder.projectCategory),
  }
  const form = useForm({
...
const dictionary = {
  fr: {
    ...
    description:
      "Créez une nouvelle catégorie pour organiser vos projets. Il est nécessaire de remplir tous les champs pour chacune des langues.",
    "image-label": "Image",
    ...
    "order-label": "Priorité d'affichage",
    "order-info": "La priorité d'affichage de la catégorie sera utilisée pour trier les catégories dans l'interface.",
  ...
  en: {
    ...
    description:
      "Create a new category to organize your projects. It is necessary to fill in all fields for each language.",
    "image-label": "Image",
    ...
    "order-info": "The display order of the category will be used to sort the categories in the interface.",
  ...
  de: {
    ...
    description:
      "Erstellen Sie eine neue Kategorie, um Ihre Projekte zu organisieren. Alle Felder müssen für jede Sprache ausgefüllt werden.",
    "image-label": "Bild",
    ...
    "order-info":
      "Die Anzeigereihenfolge der Kategorie wird verwendet, um die Kategorien in der Benutzeroberfläche zu sortieren.",
