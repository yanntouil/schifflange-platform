...
export const LifetimeAnalytics: React.FC<React.ComponentProps<"div">> = ({ className, ...props }) => {
  ...
  const trackingIds = React.useMemo(() => A.map(projects, (project) => project.tracking.id) as string[], [projects])
  const { data } = useSWR(
    { fetch: () => traces.stats({ trackingIds }), key: useMemoKey("projects-stats", { trackingIds }) },
