...
export const FormMediaPdfs: React.FC<FormMediaPdfsProps> = ({ classNames, className, noWrapper = false, ...props }) =>
  noWrapper ? (
      ...
      <DndContext collisionDetection={closestCenter} onDragEnd={handleDragEnd} sensors={sensors}>
        <SortableContext items={value} strategy={rectSortingStrategy}>
          <ul className='grid grid-cols-3 gap-2'>
            {A.map(value, (file) => (
              <MediaPdf key={file.id} {...{ file, onChange, onRemove, onKeyDown, onOpenInfo, focusId, setFocusId }} />
            ))}
...
const MediaPdf: React.FC<MediaPdfProps> = ({
  ...
  return (
    <li
      ...
      style={style}
      className={cx(
        "border-input bg-primary/5 relative flex aspect-[3/4] max-h-full w-auto max-w-full items-center justify-center rounded-md border transition-opacity",
