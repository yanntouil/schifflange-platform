...
import { EllipsisVertical, FileInput, FileMinus, ImageOff, ImagePlus, Info } from "lucide-react"
 ...
 * FormMediaImage
 */
type FormMediaImageProps = FieldMediaImageProps & {
  noWrapper?: boolean
} & FormGroupProps & {
...
export const FormMediaImage: React.FC<FormMediaImageProps> = ({
  classNames,
  className,
  noWrapper = false,
  ...props
  ...
  noWrapper ? (
    <Form.Fields name={props.name}>
      <FieldMediaImage {...extractInputProps({ ...props })} className={cx(classNames?.input, className)} />
    </Form.Fields>
  ) : (
 ...
 * FieldMediaImage
 */
const FieldMediaImage: React.FC<FieldMediaImageProps> = ({ ...props }) => {
  const { setFieldValue, value, id } = useFieldContext<Api.MediaFileWithRelations | null>()

  return <InputMediaImage value={value} onValueChange={setFieldValue} id={id} {...props} />
}

/**
 * MediaImageInput
 */
type FieldMediaImageProps = {
  ratio?: string
  className?: string
  fit?: string
  contextKey?: string
}
const InputMediaImage: React.FC<
  FormMediaImageProps & {
      ...
      {G.isNotNullable(value) ? (
        <>
          <Ui.Image
            ...
            alt={translate(value, servicePlaceholder.mediaFile).name}
            classNames={{ wrapper: cx("size-full", ratio) }}
