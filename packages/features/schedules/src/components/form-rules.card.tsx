...
import { A, cxm, G, T } from "@compo/utils"
import { Trash2, X } from "lucide-react"
import React from "react"
import { FormScheduleRule, makeRecurringFrequency } from "../utils"

/**
 * Schedule Rule Card Component
 */
export type ScheduleRuleCardProps = {
  rule: FormScheduleRule
  index: number
  canDelete: boolean
  onUpdate: (updates: Partial<FormScheduleRule>) => void
  onRemove: () => void
...
export const ScheduleRuleCard: React.FC<ScheduleRuleCardProps> = ({
  ...
  const endDateError = React.useMemo(
    () =>
      G.isNotNullable(rule.endDate) && G.isNotNullable(rule.startDate) && T.isBefore(rule.endDate, rule.startDate)
        ? _("end-date-invalid")
        : undefined,
    [rule.endDate, rule.startDate, _]
  )

  const endDateTimeError = React.useMemo(
    () =>
      G.isNotNullable(rule.endDateTime) &&
      G.isNotNullable(rule.startDateTime) &&
      T.isBefore(rule.endDateTime, rule.startDateTime)
        ? _("end-datetime-invalid")
        : undefined,
    [rule.endDateTime, rule.startDateTime, _]
  )
      ...
      <Ui.Card.Content className='space-y-4'>
        ...
        <div className='flex items-center justify-between space-x-2'>
          ...
          <Ui.Switch
            id={`${id}-allDay`}
            checked={rule.allDay}
            onCheckedChange={(checked) => onUpdate({ allDay: checked === true })}
          />
        </div>
          ...
          <div className='grid gap-4 @3xl/schedule-card:grid-cols-2'>
            ...
            >
              <Ui.Datepicker
                ...
                highlightWeekends
                highlightHolidays
                minDate={rule.startDate || undefined}
              />
            </FieldGroup>
          </div>
        ) : (
        ...
        <div className='space-y-3'>
          <div className='flex items-center justify-between space-x-2'>
            <div className='flex items-center gap-2'>
              <Ui.Label htmlFor={`recurring-${rule.id}`} className='grow'>
                {_("recurring-label")}
              </Ui.Label>
            ...
            <Ui.Card.Root>
              <Ui.Card.Content className='space-y-4 pt-6'>
                ...
                >
                  ...
                  >
                    {freqOptions.map(({ label, value, description }) => (
                      ...
                      >
                        <span className='group-data-[state=on]:text-primary-foreground text-sm font-medium'>
                          {label}
                        </span>
                        <span className='text-muted-foreground group-data-[state=on]:text-primary-foreground text-xs'>
                          {description}
                        </span>
                      </Primitives.ToggleGroup.Item>
                    ))}
                ...
                {rule.freq === "WEEKLY" && (
                  ...
                  >
                    <Primitives.ToggleGroup.Root
                      type='multiple'
                      id={`${id}-weekDays`}
                      value={rule.byWeekday.map(String)}
                ...
                <div className='grid gap-4 pt-4 @3xl/schedule-card:grid-cols-2'>
                  ...
                  >
                    <InputNumber min={1} value={rule.interval} onValueChange={(interval) => onUpdate({ interval })} />
                  </FieldGroup>
                  ...
                  >
                    <Ui.Datepicker
                      ...
                      clearable
                      minDate={rule.startDate || rule.startDateTime || undefined}
                      highlightWeekends
                      highlightHolidays
                    />
                  </FieldGroup>
                ...
                {/* Excluded Dates */}
                <FieldGroup
                  label={_("excluded-dates-label")}
                  id={`${id}-excludedDates`}
                  labelAside={
                    <Form.Info title={_("excluded-dates-label")} content={_("excluded-dates-info-content")} />
                  }
                >
                  <div className='grid grid-cols-3 gap-2'>
                    {A.mapWithIndex(rule.excludedDates, (index, date) => (
                      <div
                        key={index}
                        className={cxm(
                      ...
                      >
                        <Ui.Datepicker
                          ...
                          onValueChange={(newDate) =>
                            newDate && onUpdate({ excludedDates: A.replaceAt(rule.excludedDates, index, newDate) })
                          }
                          className='grow border-transparent bg-transparent'
                          placeholder={_("excluded-dates-placeholder")}
                          minDate={rule.startDate || rule.startDateTime || undefined}
                          highlightWeekends
                          highlightHolidays
                        />
                        <Ui.Button
                          ...
                          size='sm'
                          onClick={() => {
                            const updated = A.filterWithIndex(rule.excludedDates, (i) => i !== index)
                            onUpdate({ excludedDates: updated })
                          }}
                          aria-label={_("excluded-dates-remove-aria")}
                        >
                          <X className='h-4 w-4' />
                        </Ui.Button>
                      </div>
                    ...
                    <Ui.Button
                      ...
                      size='sm'
                      onClick={() => {
                        onUpdate({ excludedDates: [...rule.excludedDates, new Date()] })
                      }}
                    >
                      {_("excluded-dates-add-button")}
                    </Ui.Button>
                  </div>
                </FieldGroup>
              </Ui.Card.Content>
...
const dictionary = {
  fr: {
    ...
    "start-date-info-content":
      "Date à laquelle l'événement commence. Pour un événement sur plusieurs jours, indiquez le premier jour.",
  ...
  en: {
    ...
    "end-datetime-info-content":
      "Exact moment when the event ends. If the event ends the next day, select the next day's date.",
  ...
  de: {
    ...
    "start-date-info-content":
      "Datum, an dem die Veranstaltung beginnt. Für eine mehrtägige Veranstaltung geben Sie den ersten Tag an.",
    ...
    "start-datetime-info-content":
      "Genauer Zeitpunkt, an dem die Veranstaltung beginnt. Wählen Sie das Datum und dann die Uhrzeit.",
    ...
    "frequency-info-content":
      "Wählen Sie, wie oft die Veranstaltung wiederholt wird: täglich, wöchentlich, monatlich oder jährlich.",
