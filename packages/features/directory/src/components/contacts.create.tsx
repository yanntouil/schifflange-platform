 ...
 */
export const ContactsCreateDialog: React.FC<Ui.QuickDialogProps<void, Api.Contact> & { noRedirect?: boolean }> = (
  props
) => {
 ...
 * DialogForm
 */
const DialogForm: React.FC<Ui.QuickDialogSafeProps<void, Api.Contact> & { noRedirect?: boolean }> = ({
  close,
  mutate,
  noRedirect,
}) => {
  ...
  const initialValues = {
    ...
    lastName: "",
    politicalParty: "",
    portraitImage: makeFormFileValue(null),
    squareImage: makeFormFileValue(null),
    phones: [],
    emails: [],
    extras: [],
    translations: useFormTranslatable<Api.ContactTranslation, Partial<Api.ContactTranslation>>(
      [],
      servicePlaceholder.contact
    ),
  }
  const form = useForm({
...
const dictionary = {
  ...
  fr: {
    ...
    description: "Ajouter un nouveau contact à l'annuaire.",
    submit: "Créer le contact",
    created: "Contact créé avec succès",
    failed: "Échec de la création du contact",
  ...
  de: {
    ...
    description: "Einen neuen Kontakt zum Verzeichnis hinzufügen.",
    submit: "Kontakt erstellen",
    created: "Kontakt erfolgreich erstellt",
    failed: "Kontakt konnte nicht erstellt werden",
