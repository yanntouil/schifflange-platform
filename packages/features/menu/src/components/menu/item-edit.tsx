 ...
 * EditDialog
 */
export const EditDialog: React.FC<Ui.QuickDialogProps<Api.MenuItemWithRelations, Api.MenuItemWithRelations>> = ({
  item,
  ...props
}) => {
 ...
 * DialogForm
 */
const DialogForm: React.FC<Ui.QuickDialogSafeProps<Api.MenuItemWithRelations, Api.MenuItemWithRelations>> = ({
  close,
  item,
}) => {
  const { _ } = useTranslation(dictionary)
  ...
  const form = useForm({
    values: makeItemValues(item, languages),
    onSubmit: async ({ values }) => {
      const payload = getItemPayload(values)
      match(await service.id(swr.menu.id).items.id(item.id).update(payload))
        .with({ ok: true }, ({ data }) => {
