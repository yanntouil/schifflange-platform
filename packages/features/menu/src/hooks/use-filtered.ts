...
export const useFilteredForwards = (forwards: Api.Forward[]) => {
  const { serviceKey } = useMenusService()
  ...
  const [matchable, matchIn] = useMatchable<Api.Forward>(`${prefixedKey}-search`, [
    (item) => translate(getSlugSeo(item.slug), servicePlaceholder.seo).title,
    (item) => item.path,
    (item) => item.slug.path,
  ])
  ...
  const [sortable, sortBy] = useSortable<Api.Forward>(
    `${prefixedKey}-sort`,
    {
      title: [(forward) => translate(getSlugSeo(forward.slug), servicePlaceholder.seo).title, "asc", "alphabet"],
