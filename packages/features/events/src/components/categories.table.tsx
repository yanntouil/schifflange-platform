 ...
 * CategoriesTable
 */
export const CategoriesTable: React.FC<{ categories: Api.EventCategory[] }> = ({ categories }) => {
  const { _ } = useTranslation(dictionary)
  const table = useReactTable<Api.EventCategory>({
    data: categories,
    columns,
...
// Column components
type ColumnProps = { row: Row<Api.EventCategory> }
...
const ColumnEventsCount: React.FC<ColumnProps> = ({ row }) => {
  const { _ } = useTranslation(dictionary)
  const category = row.original
  return (
    <span className='inline-flex items-center gap-2'>
      <FolderIcon aria-hidden className='text-blue-600 size-4' />
      {_("events-count", { count: category.totalEvents || 0 })}
    </span>
  )
}
 ...
 * Table columns
 */
const columns: ColumnDef<Api.EventCategory>[] = [
  {
    id: "select",
    size: 48,
    minSize: 48,
  ...
  {
    id: "eventsCount",
    header: "events-count-label",
    size: 120,
    minSize: 100,
    maxSize: 140,
    accessorFn: (category) => category.totalEvents || 0,
    cell: ({ row }) => <ColumnEventsCount row={row} />,
  },
  {
    id: "updatedBy",
    ...
    minSize: 150,
    maxSize: 220,
    accessorFn: (category) => category.updatedBy.id,
...
const dictionary = {
  ...
  de: {
    "title-label": "Titel",
    "events-count-label": "Veranstaltungen",
