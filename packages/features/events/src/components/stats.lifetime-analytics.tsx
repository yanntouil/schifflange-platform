...
export const LifetimeAnalytics: React.FC<React.ComponentProps<"div">> = ({ className, ...props }) => {
  ...
  const trackingIds = React.useMemo(() => A.map(events, (event) => event.tracking.id) as string[], [events])
  const { data } = useSWR(
    { fetch: () => traces.stats({ trackingIds }), key: useMemoKey("events-stats", { trackingIds }) },
