 ...
 */
export const CategoriesEditDialog: React.FC<Ui.QuickDialogProps<Api.EventCategory>> = ({ item, ...props }) => {
...
const DialogForm: React.FC<Ui.QuickDialogSafeProps<Api.EventCategory>> = ({ item, close, mutate }) => {
  const { _ } = useTranslation(dictionary)
  const { service } = useEventsService()
  const form = useForm({
    values: {
      translations: useFormTranslatable(item.translations, servicePlaceholder.eventCategory),
    },
