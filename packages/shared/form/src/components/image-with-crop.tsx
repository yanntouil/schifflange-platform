 ...
 * default config
 */
const defaultAccept = config.acceptCroppableImageExtensions
const defaultAspect = "aspect-video"

/**
 * FormImageWithCrop
 */
export type FormImageWithCropProps = ImageWithCropInputProps &
  Omit<FormGroupProps, "classNames"> & {
    classNames?: FormGroupClassNames<{
      input?: string
    }>
  }
export const FormImageWithCrop: React.FC<FormImageWithCropProps> = ({ ...props }) => (
  <FormGroup {...extractGroupProps(props)}>
    <ImageWithCropInput {...extractInputProps<FormGroupProps>(props)} className={props.classNames?.input} />
  </FormGroup>
)

/**
 * FormInputImageWithCrop
 */
type ImageWithCropInputProps = React.ComponentProps<"div"> & {
  aspect?: string
  placeholder?: string
  disabled?: boolean
  accept?: string[]
  min?: number
  max?: number
  cropAspect?: number // ex: 16/9, 1/1, 4/3
  cropShape?: "rect" | "round"
}
const ImageWithCropInput = React.forwardRef<HTMLDivElement, ImageWithCropInputProps>((props, ref) => {
  ...
  // Handle crop complete from dialog
  const onCropComplete = React.useCallback(
    (croppedFile: File) => {
      setFieldValue({ ...value, file: croppedFile })
        ...
        {G.isNotNullable(imagePath) ? (
          <>
            ...
            <div className='absolute inset-0' aria-hidden>
              <Ui.Image src={imagePath} className='h-full w-full rounded-sm bg-muted object-cover'>
