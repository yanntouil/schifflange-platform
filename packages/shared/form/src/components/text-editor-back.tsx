    ...
    <div className='text-sm'>
      ...
      <EditorContent
        ...
        className={cxm(
          ...
          "h-auto resize-none outline-none scrollbar-thin scrollbar-track-transparent scrollbar-thumb-transparent [&>.tiptap]:px-3 [&>.tiptap]:py-3 [&>.tiptap]:outline-none",
          prose,
          className
        )}
      />
 ...
 * Editor bubble menu
 */
export const EditorBubbleMenu: React.FC<{ editor: Editor }> = ({ editor }) => {
  ...
  return (
    <>
      <BubbleMenu
        ...
        className={cxm(
          "flex gap-0.5 rounded-lg border border-border bg-background p-1 shadow-lg",
          "animate-in fade-in-0 zoom-in-95"
        )}
      >
        <Ui.DropdownMenu.Root modal>
...
const LinkDialog: React.FC<{ editor: Editor; open: boolean; onOpenChange: (open: boolean) => void }> = (props) => {
  ...
  return (
    <Ui.QuickDialog
      ...
      classNames={{ content: "sm:max-w-xl", header: "z-10", close: "z-10" }}
      sticky
    >
      <LinkForm {...props} />
    </Ui.QuickDialog>
  ...
  return (
    ...
    >
      ...
      <Ui.QuickDialogStickyFooter>
        <Ui.Button type='submit' disabled={disabled} className='w-full'>
          {_("link.save")}
        </Ui.Button>
      </Ui.QuickDialogStickyFooter>
    </form>
...
const FormatMenu: React.FC<{ editor: Editor }> = ({ editor }) => {
  ...
  const activeFormat = getFormat(editor)
  return (
    <Ui.DropdownMenu.Content onCloseAutoFocus={(e) => e.preventDefault()} className='min-w-[248px]'>
