import {
  DeepPartial as DeepPartialE,
  Head as HeadE,
  MarkOptional as MarkOptionalE,
  MarkRequired as MarkRequiredE,
  Merge as MergeE,
  MergeN as MergeNE,
  Tail as TailE,
} from "ts-essentials"
...
export type ActiveNullable<V> = { active: V | null }

export type ById<L extends { id: string }, R = void> = { [key: string]: R extends void ? L : R }
...
// functions
export type Endo<V> = (v: V) => V // endofunction
export type Predicate<V> = (v: V) => boolean // function predicate
export type Argless = () => void // argless function
export type Argfull = (...args: unknown[]) => void // argfull function
...
export type Indexed<V> = V & { index: number }

export type Props<Fn extends UnknownFunction> = LastParameter<Fn>
export type SpecifyProps<Fn extends UnknownFunction, T = Props<Fn>, K extends keyof T = keyof {}> = Specify<
  Props<Fn> & T,
  K
>
export type DistributeSpecify<U, K extends keyof U> = U extends any ? Specify<U, K> : never

export type Last<T extends ImplicitAny[]> = T extends [...infer _, infer L]
  ? L
...
export type As<T> = T extends object ? { [K in keyof T]: As<T[K]> } : T
...
export type Maker<Type extends Record<string, any> = Record<string, any>, Values = Partial<Type>, Config = {}> = (
  values: Values,
  config?: Config
) => Type
