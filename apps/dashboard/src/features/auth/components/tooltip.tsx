...
import { everyAreNotEmpty, makeColorsFromString, oneIsNotEmpty, placeholder } from "@compo/utils"
...
export const UserTooltip = React.forwardRef<HTMLSpanElement, Props>(
  ({ user, children, displayUsername = true, onClick, ...props }, ref) => {
    ...
    const fullName = user ? placeholder(`${user.profile.firstname} ${user.profile.lastname}`, _("user-anonymous")) : _("user-unknown")
