...
export const OverviewCard: React.FC<React.ComponentProps<"div">> = ({ className, ...props }) => {
  ...
  const totalContent = pages.length + articles.length + projects.length
  const totalPublished =
    pages.filter((p) => p.state === "published").length +
    articles.filter((a) => a.state === "published").length +
    projects.filter((p) => p.state === "published").length
  const totalDrafts =
    pages.filter((p) => p.state === "draft").length +
    articles.filter((a) => a.state === "draft").length +
    projects.filter((p) => p.state === "draft").length
