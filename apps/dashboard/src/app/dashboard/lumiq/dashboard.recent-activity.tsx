...
export const RecentActivityCard: React.FC<React.ComponentProps<"div">> = ({ className, ...props }) => {
  ...
  const recentContent = React.useMemo(() => {
    ...
    const recentProjects = projects.filter((p) => new Date(p.updatedAt) > oneWeekAgo)

    return {
      total: recentPages.length + recentArticles.length + recentProjects.length,
      ...
      <Ui.Card.Content>
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2 text-center">
            ...
            <div>
              <p className="text-lg font-semibold">{recentContent.projects}</p>
              <p className="text-muted-foreground text-xs">{_("projects")}</p>
            </div>
          </div>
        </div>
      </Ui.Card.Content>
