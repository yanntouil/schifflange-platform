...
import { Form, FormInfo, FormReorderableList, FormSimpleSwitch, Label, useForm, useFormContext } from "@compo/form"
...
import { A, D, O, pipe, placeholder, prependHttp } from "@compo/utils"
...
const FormCard: React.FC<{ id: string; index: number }> = ({ id, index }) => {
  ...
  const { displayLink } = props.cards[id]
  ...
  const hasType = React.useMemo(() => props.template === "template-1", [])
  const hasNoText = React.useMemo(() => props.template === "template-3", [])
  return (
    <div className='space-y-4'>
      <Form.Fields names={["props", "cards", id]}>
        ...
        {hasType && (
          <Form.Select
            name='type'
            label={_("card.type-label")}
            placeholder={_("card.type-placeholder")}
            labelAside={<Form.Localized title={_("card.type-label")} content={_("card.type-info")} />}
            options={typeOptions}
          />
        )}
      </Form.Fields>
      ...
      <Ui.Collapsible.Root className='space-y-4' open={displayLink}>
        <Form.Fields names={["props", "cards", id]}>
          <div className='flex gap-6 items-center py-2'>
            <Label className='grow'>{_("card.display-link-label")}</Label>
            <FormSimpleSwitch name='displayLink' size='sm' classNames={{ switch: "" }} />
          </div>
        </Form.Fields>
        <Ui.Collapsible.Content className='overflow-hidden data-[state=closed]:animate-collapsible-up data-[state=open]:animate-collapsible-down'>
          <Cms.Links.FormSingle pathNames={["props", "cards", id, "link"]} asLink noText={hasNoText} />
...
const dictionary = {
  fr: {
    cards: {
      "header-title": "Cartes",
      "header-description":
        "Les cartes sont des blocs de contenu qui permettent de présenter de l'information de manière structurée",
    ...
    card: {
      "image-label": "Image",
      "type-label": "Style visuel",
      "type-placeholder": "Choisissez un style...",
      "type-info":
        "Le style détermine les couleurs et l'apparence visuelle de la carte. Choisissez celui qui correspond le mieux à votre contenu",
      "type-gold": "Gold",
      "type-finch": "Finch",
      "type-moss": "Moss",
      ...
      "display-link-label": "Ajouter un lien",
    },
    ...
    "template-1": "Carousel multiple cartes",
    "template-2": "Grille de cartes classique",
    "template-3": "Grille de cartes agenda",
  },
  en: {
    cards: {
      "header-title": "Cards",
      "header-description": "Cards are content blocks that allow you to present information in a structured way",
      title: "Card title",
    ...
    card: {
      "image-label": "Image",
      "type-label": "Visual style",
      "type-placeholder": "Choose a style...",
      "type-info":
        "The style determines the colors and visual appearance of the card. Choose the one that best matches your content",
      "type-gold": "Gold",
      "type-finch": "Finch",
      "type-moss": "Moss",
      "type-glacier": "Glacier",
      ...
      "title-info": "The main title of your card. Keep it short and impactful to grab attention",
      ...
      "display-link-label": "Add a link",
    },

    // Template
    "template-label": "Layout",
    "template-info": "Choose how to organize the content and image. You can change the layout at any time.",
    "template-1": "Multiple cards carousel",
    "template-2": "Grid of cards classic",
    "template-3": "Grid of cards agenda",
  },
  de: {
    cards: {
      "header-title": "Karten",
      "header-description": "Karten sind Inhaltsblöcke, mit denen Sie Informationen strukturiert präsentieren können",
      title: "Kartentitel",
    ...
    card: {
      "image-label": "Bild",
      "type-label": "Visueller Stil",
      "type-placeholder": "Stil auswählen...",
      "type-info":
        "Der Stil bestimmt die Farben und das visuelle Erscheinungsbild der Karte. Wählen Sie den Stil, der am besten zu Ihrem Inhalt passt",
      "type-gold": "Gold",
      "type-finch": "Finch",
      "type-moss": "Moss",
      "type-glacier": "Glacier",
      "type-white": "Weiß",
      "title-label": "Titel",
      "title-placeholder": "Kartentitel eingeben...",
    ...
    "template-1": "Mehrere Karten-Karussell",
    "template-2": "Karten-Raster klassisch",
    "template-3": "Karten-Raster agenda",
  },
}
