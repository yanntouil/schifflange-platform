...
export function Template1({ props }: TemplateProps) {
  const { title, level, subtitle, description, displayHeading, files } = props

  return (
    <Wrapper paddingY>
      <Container>
        <div className={cn("rounded-lg p-6 bg-finch-20")}>
            ...
            {files.map((file) => (
              <li key={file.id}>
                <button
                  ...
                  className='inline-flex gap-2 text-[14px] font-normal leading-normal text-[#35374F] text-left'
                >
