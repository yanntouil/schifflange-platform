...
const validatorFn = <
  ...
  U extends Record<string, unknown> = ValidationSchema<T>,
>(
  schema: U
) => {
  return (values: T): ValidationResult => {
    ...
    for (const key in schema) {
      ...
      if (fieldRules && key in values) {
        ...
        result[key] =
          fieldRules
            .map(rule => rule(fieldValue, values))
            .filter(error => (G.isString(error) ? error : null))[0] || null
      }
    }
...
const minOrEqual =
  (min: number, equal: number, error: string) => (value: string | number | any[]) =>
    (
...
//     S.isNotEmpty(value) &&
//     S.isNotEmpty(values[key]) &&
//     !T.isBefore(T.parseISO(value), T.parseISO(values[key]))
//       ? error
//       : null
...
//     S.isNotEmpty(value) &&
//     S.isNotEmpty(values[key]) &&
//     !T.isAfter(T.parseISO(value), T.parseISO(values[key]))
//       ? error
//       : null
 ...
 * exports
 */
const validate = Object.assign(validatorFn, {
  hasValidationErrors,
  min,
  max,
  isEmail,
